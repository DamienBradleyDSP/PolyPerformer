...
juce::AudioProcessorValueTreeState::ParameterLayout PolyPerformerAudioProcessor::createParameterLayout()
{
    ...
    for (int i = 0; i <= ProjectSettings::numberOfModules; i++)
    {
        ...
        layout.add(std::make_unique<juce::AudioProcessorValueTreeState::Parameter>(
            ...
            /*Min,max, optional:precision*/ juce::NormalisableRange<float>(0, 10,
                [](float rangeStart, float RangeEnd, float valueToRemap) {return valueToRemap * RangeEnd; },
                [](float rangeStart, float RangeEnd, float valueToRemap) {return valueToRemap / RangeEnd; },
                [](float rangeStart, float RangeEnd, float valueToRemap) {return valueToRemap; }),
            /*Default/Initial value*/ 0,
            /*value to text func*/ nullptr,
            /*text to value func*/ nullptr,
            /*isMetaParameter*/ false,
